...
const projects: Project[] = [
  {
    ...
    description: "Secure online assessment platform with server-enforced timed sessions and automated evaluation",
    longDescription: "Secure exam platform with authenticated users, server-enforced timed sessions, automated answer evaluation, and instant results. Backend enforces timing and submission validation to preserve assessment integrity while the frontend provides a distraction-minimized UI. Deployed as a static frontend with serverless endpoints for auth and evaluation.",
  ...
  {
    id: 2,
    title: "AK Bangles — Jewelry Brand Landing Page",
    image: "/images/hero-bg.jpg",
    description: "Responsive, product-forward landing site for AK Bangles with mobile-first performance optimizations",
    longDescription: "Commercial landing site focused on high-quality imagery, product presentation, and brand positioning. Implements responsive layouts, lightweight assets, and clear call-to-action flows to showcase both traditional and modern bangle collections. Optimized for performance and cross-device consistency to deliver a premium user experience.",
    tech: ["React", "Vite", "Responsive Design", "Tailwind", "Performance"],
    live: "https://vsiva763-git.github.io/cheery-alfajores-407f3e/",
    highlight: "Brand-focused, responsive landing experience",
  ...
  },
  {
    ...
    description: "AtmoSense: offline satellite-image PM2.5 and EPA AQI estimator using OpenCV-derived visual features",
    longDescription: "Offline image-processing pipeline that derives PM2.5/AQI estimates from single-frame satellite imagery. Extracts haze (Laplacian variance), dark-channel turbidity, histogram-entropy visibility, contrast, brightness, and saturation, then combines them via a calibrated weighting function to produce PM2.5 proxies, EPA AQI class, and confidence scores. Includes CLAHE preprocessing, heatmap overlays, and a Flask upload/visualization UI with trend charts.",
  ...
  },
  {
    ...
    description: "Industrial Energy Monitoring: edge-to-cloud telemetry with configurable sampling and threshold alerts",
    longDescription: "Edge-to-cloud energy telemetry stack where ESP8266 edge nodes sample voltage/current sensors (configurable rates), post JSON payloads over HTTP, and trigger threshold-based alerts. Cloud stack uses ThingSpeak for real-time analytics and Firebase for historical storage and audit logs, enabling remote fault detection and energy monitoring.",
  ...
  },
  {
    ...
    description: "Sewage Gas Detection: continuous multi-gas monitoring with real-time alerts and operator dashboard",
    longDescription: "Safety-focused IoT platform using electrochemical/metal-oxide sensors for continuous toxic gas monitoring in confined utility spaces. Edge firmware samples sensors, evaluates threshold crossings, and issues immediate alerts via webhooks/notifications. Web dashboard (deployed on Render) displays live telemetry and historical alert logs for auditing and response.",
  ...
  },
  {
    ...
    description: "UbicoD Transylvania: interactive mapping and client-side geolocation with Leaflet.js",
    longDescription: "Location-based web application demonstrating interactive mapping, client-side geolocation, and spatial data visualization using Leaflet.js. Implements custom map layers, POI rendering, and responsive location tracking within a modern UI. Deployed on Netlify and optimized for responsive mapping interactions.",
  ...
  },
  {
    ...
    description: "Nature Monitor: real-time environmental telemetry aggregator with trend visualization",
    longDescription: "Real-time environmental telemetry system that ingests sensor feeds (MQTT/HTTP), normalizes readings, and visualizes live and historical trends. Features time-series charts, alert thresholds, and exportable historical data to support environmental analysis and conservation efforts. Deployed on Render with real-time updates.",
  ...
  },
  {
    ...
    description: "IS-TEWS: India-specific tsunami early warning using LightGBM (AUC 0.9957) and physics filters",
    longDescription: "Research-grade ML pipeline trained on 17,038 USGS events using a LightGBM classifier (581 trees, 30-dim feature vector) achieving AUC=0.9957. A deterministic four-rule physics-informed filter enforces geophysical constraints to reduce false positives. System exposes a Flask REST API (21 endpoints), a Leaflet.js real-time seismic dashboard, and low-latency ESP8266 hardware nodes that trigger buzzer/LED/LCD alerts in under 2 seconds. Dataset and feature engineering published on Zenodo.",
